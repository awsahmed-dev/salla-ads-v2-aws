...
import { CouponCodeCard, type Voucher, type AvailableCoupon } from "@/components/shared/coupon-code-card";
import {
  CheckCircle2,
  AlertCircle,
  Rocket,
...
const FORMAT_LABELS: Record<string, string> = {
  SINGLE: "Single Image/Video", COLLECTION: "Collection Ad", STORY: "Story Ad", DYNAMIC: "Dynamic Product Ad", INFLUENCER: "Influencer Content",
};
...
const MOCK_COUPONS: AvailableCoupon[] = [
  { code: "SALLA30", title: "30% Off Salla Ads", description: "Valid for prepay campaigns only. Max 3 redemptions per user.", discountPercent: 30 },
  { code: "WELCOME50", title: "50 SAR Off First Campaign", description: "New advertiser welcome discount. One-time use.", discountAmount: 50 },
];

const MOCK_VOUCHERS: Voucher[] = [
  { id: "v1", title: "100 SAR Off Campaign", description: "Applies to campaigns with a budget over 500 SAR.", discountAmount: 100, minSpend: 500, expiresAt: "May 30, 2026" },
  { id: "v2", title: "Free Performance Boost", description: "Unlock Salla's Performance Boost at no extra cost on your first Snapchat campaign.", discountAmount: 149, minSpend: 0 },
];
      ...
      <div className="flex items-center justify-between border-b border-border px-5 py-3">
        <div className="flex items-center gap-2.5">
          <div className={cn(
            ...
            warn ? "bg-amber-100" : "bg-[#e6fff9]"
          )}>
            {warn
              ? <AlertCircle className="size-4 text-amber-500" />
              : <Icon className="size-4 text-[#004956]" />
            }
          </div>
...
export function StepReview() {
  ...
  const couponDiscount = useMemo(() => {
    ...
    const coupon = MOCK_COUPONS.find((c) => c.code === appliedCouponCode);
    if (!coupon) return 0;
    if (coupon.discountPercent) return Math.round(subtotal * coupon.discountPercent / 100);
    return coupon.discountAmount ?? 0;
  }, [appliedCouponCode, subtotal]);
  const voucherDiscount = appliedVoucherId ? (MOCK_VOUCHERS.find((v) => v.id === appliedVoucherId)?.discountAmount ?? 0) : 0;
  const totalDiscount = couponDiscount + voucherDiscount;
  const preVat = Math.max(0, subtotal + boostAmount - totalDiscount);
  const vat = Math.round(preVat * 0.15);
  const totalWithVat = preVat + vat;
  const cashbackAmount = Math.round(subtotal * CASHBACK_RATE);
  ...
  /* ---- Derived labels ---- */
  const countriesLabel = audience.countries.map((c) => COUNTRY_MAP[c] || c).join(", ") || "Not set";
  const startFormatted = budget.startDate
    ? new Date(budget.startDate).toLocaleDateString("en-SA", { month: "short", day: "numeric" })
    : "Not set";
  const endFormatted = budget.endDate
    ? new Date(budget.endDate).toLocaleDateString("en-SA", { month: "short", day: "numeric" })
    : null;
              ...
              <div key={s.label} className="flex flex-1 items-center">
                <div className="flex flex-col items-center gap-1.5">
                  <div className={cn(
                    ...
                    s.done ? "bg-emerald-500 text-white" : s.active ? "border-2 border-amber-400 bg-amber-50 text-amber-600" : "border-2 border-border bg-muted text-muted-foreground"
                  )}>
                    <s.icon className="size-4" />
                  </div>
                  <span className={cn("text-[11px] font-medium", s.done ? "text-emerald-600" : s.active ? "text-amber-600" : "text-muted-foreground")}>
                    {s.label}
                  </span>
                </div>
                {i < 2 && (
                  <div className={cn("mx-2 h-0.5 flex-1 rounded-full", s.done ? "bg-emerald-400" : "bg-border")} />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="w-full rounded-2xl border border-border bg-card overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-px bg-border sm:grid-cols-4">
            {[
              ...
              { label: "Ads", value: `${creative.ads.length} ads`, icon: ImagePlus },
            ].map((stat) => (
          ...
          <div className="border-t border-border px-5 py-3">
            <div className="grid grid-cols-1 gap-y-2 sm:grid-cols-2 sm:gap-x-6 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-foreground">{startFull}{endFull ? ` → ${endFull}` : " (ongoing)"}</span>
              </div>
              <div className="flex justify-between">
  ...
  return (
    ...
    <div className={cn("flex flex-col gap-6 lg:flex-row lg:items-start", WIZARD_FOOTER_PADDING_BOTTOM)}>
      ...
      <div className="flex flex-1 flex-col gap-4">
        ...
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            Review your campaign settings below. Click Edit on any section to make changes.
          </p>
        </div>
        ...
        >
          ...
          {objConfig.pixelRequirement !== "none" && (
            <Row
              label="Pixel"
              value={objective.pixelMode === "salla_managed" ? "Salla Managed" : objective.pixelId ? `...${objective.pixelId.slice(-8)}` : "Not set"}
              warn={objConfig.pixelRequirement === "required" && !objective.pixelId && objective.pixelMode !== "salla_managed"}
            />
          )}
        </SummaryCard>
        ...
        <SummaryCard
          ...
          onEdit={setStep}
        >
          <Row label="Countries" value={countriesLabel} warn={audience.countries.length === 0} />
          ...
          {audience.deviceOS && audience.deviceOS.length > 0 && (
            <Row label="Devices" value={audience.deviceOS.join(", ")} />
          )}
        </SummaryCard>
        ...
        <SummaryCard
          ...
          onEdit={setStep}
        >
          <Row label="Daily budget" value={`${budget.amount.toLocaleString()} SAR`} />
          <Row label="Duration" value={`${durationDays} days`} />
          <Row label="Total budget" value={`${totalBudget.toLocaleString()} SAR`} />
          <Row label="Optimization" value={GOAL_LABELS[budget.optimizationGoal] || budget.optimizationGoal} />
          <Row label="Bid strategy" value={BID_LABELS[budget.bidStrategy]} />
          {budget.bidStrategy !== "AUTO_BID" && budget.bidAmount > 0 && (
            <Row label="Bid amount" value={`${budget.bidAmount.toLocaleString()} SAR`} />
          )}
          <Row
            label="Schedule"
            value={budget.startDate ? `${startFormatted}${endFormatted ? ` → ${endFormatted}` : " (ongoing)"}` : "Not set"}
            warn={!budget.startDate}
          />
          <Row label="Payment" value={budget.paymentMethod === "prepaid" ? "Prepaid" : "Pay as You Go"} />
          {budget.performanceBoost && (
            <Row label="Performance Boost" value={<span className="text-[#004956]">Enabled (+149 SAR)</span>} />
          )}
          {autoIncrease.enabled && autoIncreaseAvailable && (
            <Row label="Auto-increase" value={`+${autoIncrease.pct}% every ${autoIncrease.intervalDays} days`} />
          )}
        </SummaryCard>
        ...
        >
          ...
          {creative.ads.length > 0 && (
            <div className="mt-2 flex flex-col gap-2">
              {creative.ads.map((ad) => (
                <div key={ad.id} className="flex items-center gap-2.5 rounded-lg border border-border/60 bg-muted/20 px-3 py-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {FORMAT_LABELS[ad.adFormat ?? "SINGLE"] || ad.adType}
                      {ad.adFormat === "SINGLE" ? ` · ${DEST_LABELS[ad.adDestination ?? "WEBSITE"] || ""}` : ""}
                      {" · "}
          ...
          {creative.ads.length === 0 && (
            <p className="mt-1 text-xs text-amber-600">No ads created yet.</p>
          )}
        </SummaryCard>
      </div>
      ...
      <div className="w-full lg:w-[340px] lg:shrink-0">
        <div className="lg:sticky lg:top-20 flex flex-col gap-3 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto">
              ...
              <div className="flex flex-col gap-1 text-xs">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">
                    {budget.type === "daily"
                      ? autoIncrease.enabled && autoIncreaseAvailable
                        ? `${dailyAmount.toLocaleString()} SAR/day × ${durationDays}d (auto-increase)`
                ...
                {totalDiscount > 0 && (
                  <div className="flex justify-between text-emerald-600">
                    <span>Savings{appliedCouponCode ? ` (${appliedCouponCode})` : ""}{appliedVoucherId ? " + voucher" : ""}</span>
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="flex items-center gap-2.5 border-b border-border px-5 py-3">
              <div className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-[#e6fff9]">
                <Sparkles className="size-3.5 text-[#004956]" />
              </div>
              <span className="text-sm font-semibold text-foreground">Savings & Offers</span>
            </div>
            <div className="px-5 py-3">
              <CouponCodeCard
                ...
                onApplyVoucher={(id) => setAppliedVoucherId(id)}
                appliedVoucherId={appliedVoucherId}
                onRemoveVoucher={() => setAppliedVoucherId(null)}
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="flex items-center gap-2.5 border-b border-border px-5 py-3">
              <div className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-[#e6fff9]">
                <Wallet className="size-3.5 text-[#004956]" />
              </div>
            ...
            <div className="px-5 py-4">
              <div className="flex gap-2.5">
                ...
                <button
                  ...
                  className={cn(
                    "flex flex-1 flex-col items-center gap-1.5 rounded-xl border-2 px-3 py-4 transition-all",
                    paymentMethod === "wallet"
                      ? "border-[#004956] bg-[#e6fff9]/60 shadow-sm"
                      : "border-border hover:border-[#004956]/30 hover:bg-[#e6fff9]/20"
                  )}
                >
                  <div className={cn(
                    "flex size-9 items-center justify-center rounded-xl",
                    paymentMethod === "wallet" ? "bg-[#004956]/10" : "bg-muted"
                  )}>
                    <Wallet className={cn("size-4.5", paymentMethod === "wallet" ? "text-[#004956]" : "text-muted-foreground")} />
                  </div>
                  <p className={cn("text-xs font-semibold", paymentMethod === "wallet" ? "text-[#004956]" : "text-foreground")}>Store Wallet</p>
                  <p className={cn("text-xs font-bold tabular-nums", walletBalance >= totalWithVat ? "text-emerald-600" : "text-amber-600")}>
                ...
                >
                  <div className={cn(
                    "flex size-9 items-center justify-center rounded-xl",
                    paymentMethod === "credit" ? "bg-[#004956]/10" : "bg-muted"
                  )}>
                    <FileText className={cn("size-4.5", paymentMethod === "credit" ? "text-[#004956]" : "text-muted-foreground")} />
                  </div>
                  <div className="flex items-center gap-1">
                    <p className={cn("text-xs font-semibold", paymentMethod === "credit" ? "text-[#004956]" : "text-foreground")}>Monthly Credit</p>
                    {!isCreditEligible && <Lock className="size-2.5 text-muted-foreground" />}
                  </div>
                  <p className={cn("text-xs font-bold tabular-nums", creditAvailable >= totalWithVat ? "text-emerald-600" : "text-amber-600")}>
              ...
              {fundsInsufficient && (
                <div className="mt-3 flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-xs font-medium text-amber-800">Need {shortfall.toLocaleString()} SAR more</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setTopUpOpen(true)}
                    className="flex items-center gap-1 rounded-lg border border-amber-300 bg-white px-3 py-1.5 text-xs font-semibold text-amber-700 transition-colors hover:bg-amber-100"
                  >
              ...
              {/* Pay-as-you-go smart tip */}
              {budget.paymentMethod === "pay_as_you_go" && (
                <div className="mt-3 flex items-start gap-2.5 rounded-xl border border-[#a4ffe5]/40 bg-[#e6fff9] px-4 py-3">
                  <div className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-[#004956]/10 mt-0.5">
                    <TrendingUp className="size-3.5 text-[#004956]" />
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-[#004956]">Pay as you go billing</p>
                    <p className="mt-0.5 text-[11px] leading-relaxed text-[#004956]/70">
                      Your wallet is charged daily. Keep your balance above {dailyAmount.toLocaleString()} SAR to avoid campaign pausing.
                    </p>
                    <button type="button" className="mt-1.5 flex items-center gap-1 text-[11px] font-semibold text-[#004956] hover:text-[#003a44]">
                      Set up auto top-up <ArrowRight className="size-3" />
                    </button>
                  </div>
                </div>
              )}
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="px-5 py-4">
              <button
                ...
                className={cn(
                  ...
                  !allPassed || fundsInsufficient
                    ? "cursor-not-allowed bg-[#004956]/20 text-[#004956]/40"
                    : launching
                      ? "bg-[#004956] text-white"
                      : "bg-[#004956] text-white shadow-md hover:bg-[#003a44] hover:shadow-lg active:scale-[0.98]"
                )}
              >
                {launching ? (
                  <>
            ...
            <div className="border-t border-border">
              <button
                ...
                className="flex w-full items-center justify-center gap-2 px-5 py-3 text-xs font-semibold text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground"
              >
