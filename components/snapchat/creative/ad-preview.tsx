...
/* ------------------------------------------------------------------ */
/*  Swipe-up bounce keyframes (injected once via style tag)           */
/* ------------------------------------------------------------------ */
...
export function SnapchatAdPreview({
  ...
  // Story snap index (internal for COMPOSITE)
  ...
  useEffect(() => { setStorySnapIdx(0); }, [activeAdIdx]);

  // Derived values from current asset
    ...
    <SectionCard className="p-4">
      ...
      <div className="relative mx-auto w-[232px] overflow-hidden rounded-[2.2rem] border-[3px] border-foreground/10 bg-black shadow-xl">
        ...
        <div className="relative flex flex-col" style={{ height: "502px" }}>
          ...
          <div className="relative z-20 px-3 pb-1 pt-1.5">
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                {headline && (
                  <p className="truncate text-[7px] leading-tight text-white/70">{headline}</p>
                )}
              </div>

              {/* Influencer partnership badge */}
            ...
            {isDynamic ? (
              ...
              <div className="flex size-full flex-col bg-black">
                <div className="flex items-center gap-1.5 bg-gradient-to-b from-black/90 to-transparent px-3 pb-6 pt-16">
                  ...
                  <span className="text-[8px] font-semibold text-amber-400">Dynamic Product Ad</span>
                </div>
                <div className="flex-1 px-1.5 pb-28">
                  <div className="grid h-full grid-cols-2 gap-[3px]">
                    {PREVIEW_PRODUCTS.slice(0, 4).map((p) => (
                      <div key={p.id} className="relative overflow-hidden rounded-lg bg-zinc-900">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={p.image} alt={p.name} className="aspect-[3/4] w-full object-cover" crossOrigin="anonymous" />
                        {activeAd?.dynamicTemplateConfig?.showSaleBadge && p.salePrice && (
                          <div className="absolute left-1 top-1 rounded bg-red-500 px-1 py-px text-[5px] font-bold text-white">SALE</div>
                        )}
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-1.5 pt-4">
                          <p className="truncate text-[6px] font-medium leading-tight text-white">{p.name}</p>
                          {activeAd?.dynamicTemplateConfig?.showPrice && (
                            <p className="text-[6px] font-bold text-amber-400">{formatSAR(p.salePrice ?? p.price)}</p>
                          )}
                        </div>
                      </div>
                    ))}
                ...
                <div className="absolute inset-x-0 bottom-32 z-10 flex justify-center">
                  <span className="rounded-full bg-black/50 px-3 py-1 text-[7px] font-medium text-white/70 backdrop-blur-sm">
                    Swipe up to browse
                  </span>
                </div>
              </div>
              ...
              <>
                {url ? (
                  ...
                  /* Influencer claimed but no local URL */
              ...
              /* Influencer unclaimed placeholder */
          ...
          {/* ── Story Tap Navigation (left/right halves) ── */}
          {isStory && storyAssets.length > 1 && (
          ...
          <div className="absolute right-2 top-[66px] z-20 flex flex-col gap-1">
            ...
            {isSnapAd && (
              <span className="rounded-full bg-zinc-500/70 px-2 py-0.5 text-[6px] font-semibold text-white backdrop-blur-sm">Awareness</span>
            )}
          </div>
      ...
      <div className="mt-3 flex flex-col gap-2.5">
        ...
        {/* Ad name + ad group dots */}
        <div className="flex flex-col items-center gap-1.5">
          {activeAd && (
            <p className="text-center text-xs text-muted-foreground">
              {isStory ? `Snap ${storySnapIdx + 1} of ${storyAssets.length}` : activeAd.name}
            </p>
          )}
          {ads.length > 1 && (
        ...
        {!isStory && totalAssets > 1 && setPreviewAssetIdx && (
